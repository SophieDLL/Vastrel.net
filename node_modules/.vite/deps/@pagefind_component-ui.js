//#region node_modules/.pnpm/@pagefind+component-ui@1.5.2/node_modules/@pagefind/component-ui/npm_dist/mjs/component-ui.mjs
...
function findNextComponentInTabOrder(fromElement, components) {
	...
	for (const component of components) {
		...
		if (!(fromElement.compareDocumentPosition(component) & Node.DOCUMENT_POSITION_FOLLOWING)) continue;
		if (!hasTabbableChild(component)) continue;
		if (closest === null || component.compareDocumentPosition(closest) & Node.DOCUMENT_POSITION_FOLLOWING) closest = component;
	}
	return closest;
}
function findPreviousComponentInTabOrder(fromElement, components) {
	...
	for (const component of components) {
		if (component.contains(fromElement)) continue;
		if (!(fromElement.compareDocumentPosition(component) & Node.DOCUMENT_POSITION_PRECEDING)) continue;
		if (!hasTabbableChild(component)) continue;
		if (closest === null || component.compareDocumentPosition(closest) & Node.DOCUMENT_POSITION_PRECEDING) closest = component;
	}
	return closest;
}
var af_exports = {};
...
var thanks_to39 = "Taylan Özgür Bildik";
var comments39 = "";
var direction39 = "ltr";
var strings39 = {
	placeholder: "Araştır",
...
function parse(tag, options = {}) {
	...
	let index = 0;
	if (tag === null || tag === void 0) throw new Error("Expected string, got `" + tag + "`");
	if (own.call(normal, value)) {
		const replacement = normal[value];
		if ((options.normalize === void 0 || options.normalize === null || options.normalize) && typeof replacement === "string") return parse(replacement);
	...
	if (index > 1 && index < 9) {
		...
		if (index < 4) {
			...
			while (value.charCodeAt(index) === 45 && isAlphabetical(value.charCodeAt(index + 1)) && isAlphabetical(value.charCodeAt(index + 2)) && isAlphabetical(value.charCodeAt(index + 3)) && !isAlphabetical(value.charCodeAt(index + 4))) {
				if (groups > 2) return fail(index, 3, "Too many extended language subtags, expected at most 3 subtags");
		...
		while (value.charCodeAt(index) === 45) {
			...
			while (isAlphanumerical(value.charCodeAt(offset))) {
				if (offset - start > 7) return fail(offset, 1, "Too long variant, expected at most 8 characters");
				offset++;
			}
		...
		while (value.charCodeAt(index) === 45) {
			if (value.charCodeAt(index + 1) === 120 || !isAlphanumerical(value.charCodeAt(index + 1)) || value.charCodeAt(index + 2) !== 45 || !isAlphanumerical(value.charCodeAt(index + 3))) break;
			let offset = index + 2;
			let groups = 0;
			while (value.charCodeAt(offset) === 45 && isAlphanumerical(value.charCodeAt(offset + 1)) && isAlphanumerical(value.charCodeAt(offset + 2))) {
				...
				groups++;
				while (isAlphanumerical(value.charCodeAt(offset))) {
					if (offset - start > 7) return fail(offset, 2, "Too long extension, expected at most 8 characters");
					offset++;
				}
			}
	...
	if (index === 0 && value.charCodeAt(index) === 120 || value.charCodeAt(index) === 45 && value.charCodeAt(index + 1) === 120) {
		...
		while (value.charCodeAt(offset) === 45 && isAlphanumerical(value.charCodeAt(offset + 1))) {
			...
			offset = start;
			while (isAlphanumerical(value.charCodeAt(offset))) {
				if (offset - start > 7) return fail(offset, 5, "Too long private-use area, expected at most 8 characters");
				offset++;
			}
...
function getTranslations(langCode) {
	...
	const keys = [];
	if (parsed.language && parsed.script && parsed.region) keys.push(`${parsed.language}-${parsed.script}-${parsed.region}`);
	if (parsed.language && parsed.region) keys.push(`${parsed.language}-${parsed.region}`);
	if (parsed.language) keys.push(parsed.language);
	for (const key of keys) if (translations[key]) return translations[key];
...
function interpolate(str, replacements = {}, locale) {
	...
	for (const [placeholder, value] of Object.entries(replacements)) {
		...
		result = result.replace(new RegExp(`\\[${placeholder}\\]`, "g"), display);
	}
	return result;
}
...
var Announcer = class {
	...
	*/
	announce(message, priority = "polite") {
		...
		if (priority === "polite") this.politeIndex = currentIndex === 0 ? 1 : 0;
		else this.assertiveIndex = currentIndex === 0 ? 1 : 0;
	...
	*/
	clear() {
		...
		for (const priority of ["polite", "assertive"]) for (const region of this.regions[priority]) region.textContent = "";
	}
	/**
...
try {
	if (document?.currentScript && document.currentScript.tagName.toUpperCase() === "SCRIPT") {
		const match = new URL(document.currentScript.src).pathname.match(/^(.*\/)(?:pagefind[-_])?component[-_]?ui.js.*$/);
		if (match) scriptBundlePath = match[1];
	}
} catch (e) {
...
var Instance = class {
	...
	generateId(prefix, length = 2) {
		...
		const randomSeg = (len = 3) => {
			let word = "";
			for (let i = 0; i < len; i++) word += idChars[Math.floor(Math.random() * 6)];
			return word;
		};
		const id = `${prefix}-${this.name !== "default" ? `${this.name}-` : ""}${Array.from({ length }, () => randomSeg()).join("-")}`;
		if (this.generatedIds.has(id) || document.getElementById(id)) return this.generateId(prefix, length + 1);
	...
	_registerComponent(component, type, subtype = null, capabilities = {}) {
		...
		if (!this._languageSet) this.setLanguage();
	...
	getUtilities(subtype = null, requiredCapability = null) {
		...
		if (subtype !== null) utilities = utilities.filter((u) => u.componentSubtype === subtype);
		if (requiredCapability) utilities = utilities.filter((c) => c.capabilities?.[requiredCapability]);
		return utilities;
	}
	/**
	...
	*/
	deregisterShortcut(label, owner) {
		this.activeShortcuts = this.activeShortcuts.filter((s) => !(s.label === label && s.owner === owner));
		this.notifyShortcutsChanged();
	}
	/**
	...
	*/
	notifyShortcutsChanged() {
		this.getUtilities("keyboard-hints").forEach((h) => h.render?.());
	}
	/**
	* Focus the first result in the next keyboard-navigable results component.
	*/
	focusNextResults(fromElement) {
		const resultsComponent = findNextComponentInTabOrder(fromElement, this.getResults("keyboardNavigation"));
	...
	*/
	focusPreviousInput(fromElement) {
		const inputComponent = findPreviousComponentInTabOrder(fromElement, this.getInputs("keyboardNavigation"));
	...
	*/
	setLanguage(langCode) {
		if (!langCode) langCode = document?.documentElement?.lang || "en";
	...
	*/
	announce(key, replacements = {}, priority = "polite") {
		...
		if (message) this._announcer.announce(message, priority);
	}
	/**
	...
	*/
	...
	on(event, callback, owner = null) {
		if (!this.__hooks__[event]) {
			...
			console.error(`[Pagefind Component UI]: Unknown event type ${event}. Supported events: [${supportedEvents}]`);
			return;
		}
		if (typeof callback !== "function") {
			console.error(`[Pagefind Component UI]: Expected callback to be a function, received ${typeof callback}`);
			return;
		}
		if (owner) {
			const existingIndex = this.__hooks__[event].findIndex((h) => typeof h === "object" && h.owner === owner);
			if (existingIndex !== -1) {
				this.__hooks__[event][existingIndex] = {
					callback,
					owner
				};
				return;
			}
			this.__hooks__[event].push({
				callback,
				owner
			});
		} else this.__hooks__[event].push(callback);
	}
	triggerLoad() {
	...
	__dispatch__(e, ...args) {
		this.__hooks__[e]?.forEach((hook) => {
			...
			else if (hook?.callback) hook.callback(...args);
		});
	}
	async __clear__() {
	...
	async __search__(term, filters) {
		...
		if ((!term || !term.length) && !this.faceted) return this.__clear__();
	...
	async __doLoad__() {
		...
		try {
			...
			console.error(e);
			console.error([`Pagefind couldn't be loaded from ${this.options.bundlePath}pagefind.js`, `You can configure this by passing a bundlePath option to the Pagefind Component UI`].join("\n"));
			if (document?.currentScript && document.currentScript.tagName.toUpperCase() === "SCRIPT") console.error(`[DEBUG: Loaded from ${document.currentScript?.src ?? "bad script location"}]`);
			else console.error("no known script location");
			...
			if (!this.hasAnnouncementCapability()) this.announce("error_search", {}, "assertive");
			return;
		}
		const imported_pagefind = pagefindModule.createInstance(this.pagefindOptions || {});
		for (const index of this.options.mergeIndex) {
			if (!index.bundlePath) throw new Error("mergeIndex requires a bundlePath parameter");
		...
		if (this.faceted && this.__searchID__ === 0) this.triggerSearch("");
	}
	/**
	...
	*/
	getDisplaySubResults(result, limit = 3) {
		...
		const subResults = result.sub_results[0]?.url === (result.meta?.url || result.url) ? result.sub_results.slice(1) : result.sub_results;
...
var InstanceManager = class {
	...
	detectBundlePath() {
		try {
			if (document?.currentScript && document.currentScript.tagName.toUpperCase() === "SCRIPT") {
				const scriptPath = new URL(document.currentScript.src).pathname.match(/^(.*\/)(?:pagefind[-_])?.*\.js.*$/);
				if (scriptPath) return scriptPath[1];
			}
		} catch (e) {}
		return "/pagefind/";
	}
...
function getInstanceManager() {
	if (!instanceManager) instanceManager = new InstanceManager();
	return instanceManager;
}
function configureInstance(name, options) {
	const manager = getInstanceManager();
	if (manager.hasInstance(name)) {
		console.warn(`[Pagefind Component UI]: Instance "${name}" already exists, configuration ignored`);
...
var ev = (e, ctx) => {
	...
	if (e.t === "V") {
		...
		for (const k of e.path) {
			if (v == null || !has(v, k)) return void 0;
			v = v[k];
		}
		return v;
	}
...
var rn = (nodes, ctx) => {
	...
	for (const n of nodes) {
		...
		if (n.t === "I") {
			...
			else if (typeof v === "object" && v !== null) out += "[Error: cannot render object]";
			else {
				const s = String(v ?? "");
				out += n.raw ? s : esc(s);
			}
		...
		if (n.t === "F") {
			let matched = false;
			for (const b of n.branches) if (truthy(ev(b.cond, ctx))) {
				out += rn(b.body, ctx);
				matched = true;
				break;
			}
...
var compile = (tmpl) => {
	...
	const parseExpr = () => {
		...
		if (ch === "\"" || ch === "'") {
			...
			while (pos < src.length && (/[0-9]/.test(src[pos]) || src[pos] === "." && !dot++)) s += src[pos++];
			expr = s === "-" || s === "." || s === "" ? {
				t: "V",
				path: [s || "-"]
			} : {
			...
			else if (name === "false") expr = {
				t: "L",
				val: false
			};
			else if (name === "null") expr = {
				t: "L",
				val: null
			};
			else {
...
var PagefindElement = class extends HTMLElement {
	...
	connectedCallback() {
		...
		this.init();
		if (this.register && typeof this.register === "function") this.register(this.instance);
	}
	...
	attributeChangedCallback(name, oldValue, newValue) {
		...
		if (newValue === "false") this[prop] = false;
		else if (newValue === "true") this[prop] = true;
		else if (newValue === null || newValue === void 0) this[prop] = false;
	...
	ensureId(prefix = "pagefind") {
		if (!this.id && this.instance) this.id = this.instance.generateId(prefix);
		return this.id;
	}
	init() {}
	reconcileAria() {}
	...
	showError(error) {
		...
		this.dispatchEvent(new CustomEvent("pagefind-error", {
			detail: error,
			bubbles: true,
			composed: true
		}));
...
var PagefindConfig = class extends PagefindElement {
	...
	register(instance) {
		...
		if (bundlePath) instance.options.bundlePath = bundlePath;
		const baseUrl = this.getAttribute("base-url");
		if (baseUrl) instance.pagefindOptions.baseUrl = baseUrl;
		const excerptLength = this.getAttribute("excerpt-length");
		if (excerptLength) instance.pagefindOptions.excerptLength = parseInt(excerptLength, 10);
		const lang = this.getAttribute("lang");
		if (lang) instance.setLanguage(lang);
		const metaCacheTag = this.getAttribute("meta-cache-tag");
		if (metaCacheTag) instance.pagefindOptions.metaCacheTag = metaCacheTag;
		const highlightParam = this.getAttribute("highlight-param");
		if (highlightParam) instance.pagefindOptions.highlightParam = highlightParam;
		if (this.hasAttribute("exact-diacritics")) instance.pagefindOptions.exactDiacritics = true;
		if (this.hasAttribute("no-worker")) instance.pagefindOptions.noWorker = true;
		if (this.hasAttribute("faceted")) instance.faceted = true;
		if (this.hasAttribute("preload")) instance.triggerLoad();
	}
};
if (!customElements.get("pagefind-config")) customElements.define("pagefind-config", PagefindConfig);
...
var PagefindInput = class extends PagefindElement {
	...
	readAttributes() {
		...
		if (this.hasAttribute("debounce")) this.debounce = parseInt(this.getAttribute("debounce") || "300", 10) || 300;
		if (this.hasAttribute("autofocus")) this.autofocus = this.hasAttribute("autofocus");
	}
	init() {
		this.readAttributes();
		this.render();
	...
	render() {
		...
		if (this.instance?.direction === "rtl") this.setAttribute("dir", "rtl");
		...
		if (this.autofocus) this.inputEl.setAttribute("autofocus", "autofocus");
	...
	setupEventHandlers() {
		...
		this.inputEl.addEventListener("keydown", (e) => {
			...
			if (e.key === "ArrowDown") {
				e.preventDefault();
				if (this.inputEl) this.instance?.focusNextResults(this.inputEl);
			}
		});
	...
	updateState(term) {
		if (this.clearEl) {
			if (term && term?.length) this.clearEl.removeAttribute("data-pf-suppressed");
			else this.clearEl.setAttribute("data-pf-suppressed", "true");
		}
	}
	register(instance) {
...
if (!customElements.get("pagefind-input")) customElements.define("pagefind-input", PagefindInput);
var PagefindSummary = class extends PagefindElement {
	...
	init() {
		if (this.hasAttribute("default-message")) this.defaultMessage = this.getAttribute("default-message") || "";
		this.render();
	}
	render() {
		this.innerHTML = "";
	...
	register(instance) {
		...
		instance.on("loading", () => {
			...
			const text = instance.translate("searching", { SEARCH_TERM: this.term });
	...
	update() {
		if (this.hasAttribute("default-message")) {
			...
			if (!this.term && this.containerEl) this.containerEl.textContent = this.defaultMessage;
		}
	}
};
if (!customElements.get("pagefind-summary")) customElements.define("pagefind-summary", PagefindSummary);
var templateNodes = (templateResult) => {
	if (templateResult instanceof Element) return [templateResult];
	if (Array.isArray(templateResult) && templateResult.every((r) => r instanceof Element)) return templateResult;
...
var defaultResultTemplate = compile(DEFAULT_RESULT_TEMPLATE);
var defaultPlaceholderTemplate = compile(DEFAULT_PLACEHOLDER_TEMPLATE);
...
var nearestScrollParent = (el) => {
	...
	return overflowY !== "visible" && overflowY !== "hidden" ? el : nearestScrollParent(el.parentNode);
};
var Result = class {
	constructor(opts) {
		this.result = null;
	...
	async load() {
		...
		try {
			...
			const resultNodes = templateNodes(this.resultFn(this.result, {
			...
			if (firstNode instanceof Element) firstNode.replaceWith(...resultNodes);
		} catch {
			this.loading = false;
		}
		this.onLoad?.();
...
var PagefindResults = class extends PagefindElement {
	...
	init() {
		if (this.hasAttribute("show-images")) this.showImages = this.getAttribute("show-images") !== "false";
		if (this.hasAttribute("hide-sub-results")) this.hideSubResults = this.getAttribute("hide-sub-results") !== "false";
		if (this.hasAttribute("max-sub-results")) this.maxSubResults = parseInt(this.getAttribute("max-sub-results") || "3", 10) || 3;
		if (this.hasAttribute("max-results")) this.maxResults = parseInt(this.getAttribute("max-results") || "0", 10);
		if (this.hasAttribute("link-target")) this.linkTarget = this.getAttribute("link-target");
		this.checkForTemplates();
		this.render();
	}
	checkForTemplates() {
		const resultScript = this.querySelector("script[type=\"text/pagefind-template\"]:not([data-template]), script[type=\"text/pagefind-template\"][data-template=\"result\"]");
		if (resultScript) this.compiledResultTemplate = compile((resultScript.textContent || "").trim());
		const placeholderScript = this.querySelector("script[type=\"text/pagefind-template\"][data-template=\"placeholder\"]");
		if (placeholderScript) this.compiledPlaceholderTemplate = compile((placeholderScript.textContent || "").trim());
	}
	...
	*/
	getResultRenderer() {
		...
		return (result, options) => {
			return defaultResultTemplate(this.buildTemplateData(result, options));
		};
	}
	getPlaceholder() {
		if (this.compiledPlaceholderTemplate) return this.compiledPlaceholderTemplate({});
		return defaultPlaceholderTemplate({});
	}
	render() {
		const savedScripts = [];
		this.querySelectorAll("script[type=\"text/pagefind-template\"]").forEach((s) => {
			savedScripts.push(s);
		});
		this.innerHTML = "";
	...
	appendResults(nodes) {
		if (!this.containerEl) return;
		for (const node of nodes) this.containerEl.appendChild(node);
	}
	register(instance) {
		...
		instance.on("results", (results) => {
			...
			for (const result of this.results) result.cleanup();
			...
			this.results = limitedResults.map((r, idx) => {
				...
				const result = new Result({
					...
					linkTarget: this.linkTarget,
					onLoad: () => {
						if (result.result) this.clearLoadingAnnouncement();
					}
				});
				return result;
			});
		...
		instance.on("error", (error) => {
			const err = error;
			if (this.containerEl) this.containerEl.setAttribute("aria-busy", "false");
	...
	*/
	findNeighborAnchor(current, direction45) {
		...
		const walker = document.createTreeWalker(this.containerEl, NodeFilter.SHOW_ELEMENT, { acceptNode: (node) => node.tagName === "A" ? NodeFilter.FILTER_ACCEPT : NodeFilter.FILTER_SKIP });
		...
		return {
			anchor: neighbor,
			resultIndex: this.resultIndexForNode(neighbor)
		};
	}
	/**
	...
	*/
	resultIndexForNode(node) {
		...
		let el = node;
		while (el && el.parentNode !== this.containerEl) el = el.parentNode;
	...
	setupKeyboardHandlers() {
		...
		this.containerEl.addEventListener("keydown", (e) => {
			...
			if (e.key === "ArrowDown") {
				e.preventDefault();
				const neighbor = this.findNeighborAnchor(anchor, 1);
				if (neighbor) {
					neighbor.anchor.focus();
					this.scrollToCenter(neighbor.anchor, e.repeat);
					if (neighbor.resultIndex !== -1) this.preloadAhead(neighbor.resultIndex, 1);
				} else {
					const nextResultIdx = this.resultIndexForNode(anchor) + 1;
				...
				if (neighbor) {
					...
					this.scrollToCenter(neighbor.anchor, e.repeat);
					if (neighbor.resultIndex !== -1) this.preloadAhead(neighbor.resultIndex, -1);
				} else this.instance?.focusPreviousInput(document.activeElement);
	...
	scrollToCenter(el, instant = false) {
		...
		const targetScroll = elRect.top - containerRect.top + container.scrollTop - container.clientHeight / 2 + el.offsetHeight / 2;
		container.scrollTo({
			top: targetScroll,
	...
	preloadAhead(fromIndex, direction45) {
		...
		for (let i = 1; i <= 3; i++) {
			...
			if (idx >= 0 && idx < this.results.length) {
				...
				if (result && !result.result) result.load();
			}
		}
	}
...
if (!customElements.get("pagefind-results")) customElements.define("pagefind-results", PagefindResults);
var PagefindFilterPane = class extends PagefindElement {
	...
	init() {
		if (this.hasAttribute("show-empty")) this.showEmpty = this.getAttribute("show-empty") !== "false";
		if (this.hasAttribute("expanded")) this.expanded = this.getAttribute("expanded") !== "false";
		if (this.hasAttribute("open")) this.openFilters = (this.getAttribute("open") || "").split(",").map((s) => s.trim().toLowerCase()).filter((s) => s.length > 0);
		if (this.hasAttribute("sort")) {
			...
			if ([
				...
				"count-desc",
				"count-asc"
			].includes(sortVal)) this.sortOption = sortVal;
		}
		if (this.hasAttribute("auto-open-threshold")) this.autoOpenThreshold = parseInt(this.getAttribute("auto-open-threshold") || "6", 10);
		this.render();
	}
	sortValues(values, availableValues) {
		if (this.sortOption === "default") return values;
		const sorted = [...values];
		switch (this.sortOption) {
			...
			case "count-desc":
				sorted.sort((a, b) => {
					...
					return (availableValues[b[0]] ?? b[1]) - countA;
				});
				break;
			case "count-asc": sorted.sort((a, b) => {
				return (availableValues[a[0]] ?? a[1]) - (availableValues[b[0]] ?? b[1]);
			});
		}
		return sorted;
	}
	...
	shouldGroupStartOpen(filterName, valueCount, filterCount) {
		if (this.openFilters.length > 0) return this.openFilters.includes(filterName.toLowerCase());
	...
	hasStructureChanged() {
		...
		for (const group of currentGroups) if (!renderedGroups.has(group)) return true;
		for (const [filterName, values] of Object.entries(this.totalFilters)) {
			const currentValues = new Set(Object.keys(values));
			for (const value of currentValues) if (!this.filterElements.has(`${filterName}:${value}`)) return true;
		}
		return false;
	}
	handleFiltersUpdate() {
		...
		if (!this.isRendered || this.hasStructureChanged()) this.renderFilters();
	...
	renderFilters() {
		...
		for (const filterName of filterNames) {
			...
			const group = this.renderFilterGroup(filterName, values, availableValues, filterNames.length);
			if (group) this.containerEl.appendChild(group);
		}
	...
	updateFilters() {
		for (const [key, elements] of this.filterElements) {
			...
			if (shouldShow && wasHidden) this.groupVisibleCounts.set(filterName, (this.groupVisibleCounts.get(filterName) ?? 0) + 1);
			else if (!shouldShow && !wasHidden) this.groupVisibleCounts.set(filterName, (this.groupVisibleCounts.get(filterName) ?? 1) - 1);
		...
		for (const [filterName, elements] of this.groupElements) {
			...
			if (elements.selectedCountSpan) {
				...
				} else elements.selectedCountSpan.setAttribute("data-pf-hidden", "true");
			}
	...
	renderFilterGroup(filterName, values, availableValues, filterCount) {
		...
		if (this.expanded) {
			...
			if (selectedCount > 0) selectedCountSpan.textContent = this.getSelectedText(selectedCount);
			else selectedCountSpan.setAttribute("data-pf-hidden", "true");
		...
		for (const [value, totalCount] of valueEntries) {
			...
			this.renderCheckbox(optionsContainer, filterName, value, count, isSelected, shouldShow);
		}
		this.groupVisibleCounts.set(filterName, visibleCount);
		return group;
	}
	renderCheckbox(container, filterName, value, count, isSelected, shouldShow) {
		const checkboxId = this.instance.generateId(`pf-filter-${filterName}-${value}`);
		const label = document.createElement("label");
		label.className = "pf-filter-checkbox";
		label.setAttribute("for", checkboxId);
		if (!shouldShow) label.setAttribute("data-pf-hidden", "true");
		...
		checkbox.addEventListener("change", (e) => {
			this.handleCheckboxChange(filterName, value, e.target.checked);
		});
	...
	handleCheckboxChange(filterName, value, checked) {
		if (!this.selectedFilters[filterName]) this.selectedFilters[filterName] = /* @__PURE__ */ new Set();
		if (checked) this.selectedFilters[filterName].add(value);
		else this.selectedFilters[filterName].delete(value);
		...
		if (groupElements?.selectedCountSpan) {
			...
			} else groupElements.selectedCountSpan.setAttribute("data-pf-hidden", "true");
		}
		const selectedValues = Array.from(this.selectedFilters[filterName]);
		if (selectedValues.length === 0) {
			delete this.selectedFilters[filterName];
			const filters = {};
			for (const [name, values] of Object.entries(this.selectedFilters)) filters[name] = Array.from(values);
			this.instance?.triggerFilters(filters);
		} else this.instance?.triggerFilter(filterName, selectedValues);
	}
	register(instance) {
		...
		instance.on("search", (_term, filters) => {
			...
			const f = filters;
			if (f) {
				for (const [name, values] of Object.entries(f)) if (Array.isArray(values) && values.length > 0) this.selectedFilters[name] = new Set(values);
			}
			if (this.isRendered) this.updateFilters();
		}, this);
		instance.on("error", (error) => {
			const err = error;
			this.showError({
...
if (!customElements.get("pagefind-filter-pane")) customElements.define("pagefind-filter-pane", PagefindFilterPane);
var PagefindFilterDropdown = class extends PagefindElement {
		...
		switch (this.sortOption) {
			...
			case "count-desc":
				sorted.sort((a, b) => {
					...
					return (this.availableFilters[b] ?? this.totalFilters[b] ?? 0) - countA;
				});
				break;
			case "count-asc": sorted.sort((a, b) => {
				return (this.availableFilters[a] ?? this.totalFilters[a] ?? 0) - (this.availableFilters[b] ?? this.totalFilters[b] ?? 0);
			});
		}
		return sorted;
	}
	render() {
		...
		if (!this.hideClear) {
			...
			this.clearEl.setAttribute("aria-label", (this.instance?.translate("clear_search") || "Clear") + " " + (this.getAttribute("label") || this.filterName || ""));
		...
		this.triggerEl.addEventListener("keydown", (e) => {
			if (this.isOpen) this.handleMenuKeydown(e);
			else this.handleTriggerKeydown(e);
		});
		this.isRendered = true;
	}
	toggle() {
	...
	open() {
		this.instance?.triggerLoad();
		if (this.isOpen || !this.menuEl || !this.triggerEl || !this.optionsEl) return;
		this.isOpen = true;
		if (!this.filtersLoaded) this.showLoadingState();
	...
	_handleClickOutside(event) {
		if (this.wrapperEl && !this.wrapperEl.contains(event.target)) this.close(false);
	}
		...
		switch (e.key) {
			...
			case " ":
				...
				if (this.activeIndex >= 0 && this.activeIndex < this.optionElements.length) {
					...
					if (activeOption) this.toggleOption(activeOption.value);
				}
				break;
			case "Escape":
				e.preventDefault();
			...
			case "Tab":
				this.close(false);
				break;
			default: if (e.key.length === 1 && !e.ctrlKey && !e.metaKey && !e.altKey) this.handleTypeAhead(e.key);
		}
	}
	setActiveIndex(index) {
		if (index < 0 || index >= this.optionElements.length || !this.optionsEl) return;
		if (this.focusedOptionEl) this.focusedOptionEl.classList.remove("pf-dropdown-option-focused");
	...
	scrollToCenter(el) {
		...
		const targetScroll = elTop - container.clientHeight / 2 + elHeight / 2;
		container.scrollTo({
			top: targetScroll,
			behavior: "smooth"
		});
	...
	moveActiveIndex(delta) {
		...
		else if (newIndex >= this.optionElements.length) newIndex = 0;
		this.setActiveIndex(newIndex);
	}
	handleTypeAhead(char) {
		this.typeAheadBuffer += char.toLowerCase();
		if (this.typeAheadTimeout) clearTimeout(this.typeAheadTimeout);
		const matchIndex = this.optionElements.findIndex(({ value }) => value.toLowerCase().startsWith(this.typeAheadBuffer));
		if (matchIndex >= 0) this.setActiveIndex(matchIndex);
	...
	updateOptions() {
		...
		values.forEach((value, index) => {
			...
			if (!(this.showEmpty || availableCount > 0 || isSelected)) return;
			const count = isSelected ? totalCount : availableCount;
		...
		if (this.isOpen && this.optionElements.length > 0) {
			if (this.activeIndex >= this.optionElements.length) this.setActiveIndex(this.optionElements.length - 1);
			else if (this.activeIndex < 0) this.setActiveIndex(0);
	...
	toggleOption(value) {
		...
		if (this.singleSelect) {
			if (this.selectedValues.has(value)) this.selectedValues.clear();
			else {
				this.selectedValues.clear();
				this.selectedValues.add(value);
			}
			this.close();
		} else if (this.selectedValues.has(value)) this.selectedValues.delete(value);
	...
	dispatchFilterChange() {
		...
		if (selectedArray.length === 0) this.instance?.triggerFilter(this.filterName, []);
		else this.instance?.triggerFilter(this.filterName, selectedArray);
	}
	updateBadge() {
		...
		if (count > 0) {
			...
			this.triggerEl.setAttribute("aria-label", `${label}, ${count} ${filterWord} selected`);
			if (this.clearEl) this.clearEl.removeAttribute("aria-disabled");
		} else {
			this.badgeEl.setAttribute("data-pf-hidden", "true");
			this.triggerEl.removeAttribute("aria-label");
			if (this.clearEl) this.clearEl.setAttribute("aria-disabled", "true");
		}
	}
	updateOptionStates() {
	...
	register(instance) {
		...
		instance.on("filters", (filters) => {
			...
			if (this.isRendered) this.updateOptions();
		}, this);
		instance.on("search", (_term, filters) => {
			const externalValues = filters?.[this.filterName] || [];
	...
	update() {
		...
		if (this.optionsEl) this.optionsEl.setAttribute("aria-multiselectable", this.singleSelect ? "false" : "true");
		const labelSpan = this.triggerEl?.querySelector(".pf-dropdown-trigger-label");
		if (labelSpan) labelSpan.textContent = this.getAttribute("label") || this.filterName || "";
		this.updateOptions();
	}
	cleanup() {
		document.removeEventListener("click", this._handleClickOutside);
...
if (!customElements.get("pagefind-filter-dropdown")) customElements.define("pagefind-filter-dropdown", PagefindFilterDropdown);
var PagefindModal = class extends PagefindElement {
	...
	init() {
		if (this.hasAttribute("reset-on-close")) this.resetOnClose = this.getAttribute("reset-on-close") !== "false";
		this.render();
	}
	render() {
		...
		if (hasChildren && children) children.forEach((child) => this.dialogEl.appendChild(child));
		else {
	...
	open() {
		...
		(this.instance?.getUtilities("modal-trigger") || []).forEach((t) => t.buttonEl?.setAttribute("aria-expanded", "true"));
	}
	close() {
		if (!this._isOpen || !this.dialogEl) return;
		this.dialogEl.close();
	...
	handleClose() {
		this.instance?.deregisterAllShortcuts(this);
		if (this.resetOnClose && this.instance) this.instance.triggerSearch("");
		const trigger = (this.instance?.getUtilities("modal-trigger") || [])[0];
		if (trigger && typeof trigger.handleModalClose === "function") trigger.handleModalClose();
	}
	get isOpen() {
		return this._isOpen;
	}
	...
	reconcileAria() {
		(this.instance?.getUtilities("modal-trigger") || []).forEach((t) => {
			if (t.buttonEl && this.dialogEl?.id) t.buttonEl.setAttribute("aria-controls", this.dialogEl.id);
		});
	}
	cleanup() {
		if (this.dialogEl && this._closeHandler) this.dialogEl.removeEventListener("close", this._closeHandler);
		this.instance?.deregisterAllShortcuts(this);
	}
	update() {
		if (this.hasAttribute("reset-on-close")) this.resetOnClose = this.getAttribute("reset-on-close") !== "false";
...
if (!customElements.get("pagefind-modal")) customElements.define("pagefind-modal", PagefindModal);
var _isMac = null;
function detectMac() {
...
var PagefindModalTrigger = class extends PagefindElement {
	...
	readAttributes() {
		if (this.hasAttribute("placeholder")) this._userPlaceholder = this.getAttribute("placeholder");
		if (this.hasAttribute("shortcut")) this.shortcut = this.getAttribute("shortcut") || "mod+k";
		if (this.hasAttribute("hide-shortcut")) this.hideShortcut = this.getAttribute("hide-shortcut") !== "false";
		if (this.hasAttribute("compact")) this.compact = this.getAttribute("compact") !== "false";
	...
	openModal() {
		const modal = (this.instance?.getUtilities("modal") || [])[0];
	...
	reconcileAria() {
		const modal = (this.instance?.getUtilities("modal") || [])[0];
		if (modal?.dialogEl?.id && this.buttonEl) this.buttonEl.setAttribute("aria-controls", modal.dialogEl.id);
	}
	cleanup() {
...
if (!customElements.get("pagefind-modal-trigger")) customElements.define("pagefind-modal-trigger", PagefindModalTrigger);
var PagefindModalHeader = class extends PagefindElement {
	...
	init() {
		...
		this.closeBtn.setAttribute("aria-label", this.instance?.translate("keyboard_close") || "Close");
	...
	register(instance) {
		...
		instance.on("translations", () => {
			if (this.closeBtn) this.closeBtn.setAttribute("aria-label", instance.translate("keyboard_close") || "Close");
		}, this);
	}
};
if (!customElements.get("pagefind-modal-header")) customElements.define("pagefind-modal-header", PagefindModalHeader);
...
if (!customElements.get("pagefind-modal-body")) customElements.define("pagefind-modal-body", PagefindModalBody);
...
if (!customElements.get("pagefind-modal-footer")) customElements.define("pagefind-modal-footer", PagefindModalFooter);
...
if (!customElements.get("pagefind-keyboard-hints")) customElements.define("pagefind-keyboard-hints", PagefindKeyboardHints);
...
var templateNodes2 = (templateResult) => {
	...
	console.error(`[Pagefind Searchbox]: Expected template to return HTML element or string, got ${typeof templateResult}`);
	return [];
};
var defaultResultTemplate2 = compile(`{{#if and(options.show_sub_results, sub_results)}}<div class="pf-searchbox-group" role="group" aria-label="{{ meta.title | default('Untitled') }}">{{/if}}<a class="pf-searchbox-result" id="{{ aria.result_id }}" href="{{ meta.url | default(url) | safeUrl }}" role="option" aria-selected="false" aria-labelledby="{{ aria.title_id }}"{{#if excerpt}} aria-describedby="{{ aria.excerpt_id }}"{{/if}}>
...
var defaultPlaceholderTemplate2 = compile(`<div class="pf-searchbox-result pf-searchbox-placeholder" aria-hidden="true">
...
var SearchboxResult = class {
	...
	async load() {
		...
		try {
			...
			const nodes = templateNodes2(this.renderFn(this.data));
	...
	cacheOptions() {
		...
		if (this.placeholderEl.getAttribute("role") === "group") this.cachedOptions = Array.from(this.placeholderEl.querySelectorAll("[role=\"option\"]"));
		else if (this.placeholderEl.getAttribute("role") === "option") this.cachedOptions = [this.placeholderEl];
		else this.cachedOptions = [];
	}
	cleanup() {
		if (this.observer) {
			this.observer.disconnect();
...
var PagefindSearchbox = class extends PagefindElement {
	...
	readAttributes() {
		if (this.hasAttribute("placeholder")) this._userPlaceholder = this.getAttribute("placeholder");
		if (this.hasAttribute("debounce")) this.debounce = parseInt(this.getAttribute("debounce") || "150", 10) || 150;
		if (this.hasAttribute("autofocus")) this.autofocus = this.hasAttribute("autofocus");
		if (this.hasAttribute("show-sub-results")) this.showSubResults = this.getAttribute("show-sub-results") !== "false";
		if (this.hasAttribute("max-results")) this.maxResults = parseInt(this.getAttribute("max-results") || "0", 10);
		if (this.hasAttribute("show-keyboard-hints")) this.showKeyboardHints = this.getAttribute("show-keyboard-hints") !== "false";
		if (this.hasAttribute("shortcut")) this.shortcut = this.getAttribute("shortcut") || "mod+k";
		if (this.hasAttribute("hide-shortcut")) this.hideShortcut = this.getAttribute("hide-shortcut") !== "false";
		this._keyBinding = parseKeyBinding(this.shortcut);
	}
	...
	setupEventHandlers() {
		...
		this.inputEl.addEventListener("keydown", (e) => {
			switch (e.key) {
				case "ArrowDown":
					e.preventDefault();
					if (!this.isOpen && this.inputEl?.value.trim()) this.openDropdown();
					if (this.isOpen && this.results.length > 0) this.moveSelection(1);
					break;
				case "ArrowUp":
					e.preventDefault();
					if (this.isOpen && this.results.length > 0) this.moveSelection(-1);
				...
				case "Enter":
					if (this.isOpen && this.activeIndex >= 0) {
						...
						} else this.instance?.triggerSearch(this.inputEl.value);
					}
					break;
				case "Escape":
					this.pendingNavigation = 0;
		...
		this.resultsEl.addEventListener("click", (e) => {
			if (e.target.closest("a")) this.closeDropdown();
		});
	...
	setupOutsideClickHandler() {
		this._documentClickHandler = (e) => {
			if (this.isOpen && !this.contains(e.target)) this.closeDropdown();
		};
	...
	showEmptyState() {
		...
		const noResultsText = this.instance?.translate("zero_results", { SEARCH_TERM: this.searchTerm }) || `No results for "${this.searchTerm}"`;
	...
	getOptionsForResult(result) {
		...
		if (result.placeholderEl.getAttribute("role") === "group") return Array.from(result.placeholderEl.querySelectorAll("[role=\"option\"]"));
		if (result.placeholderEl.getAttribute("role") === "option") return [result.placeholderEl];
		return [];
	}
	moveSelection(delta) {
	...
	preloadAhead(fromIndex, direction45) {
		...
		for (let i = 1; i <= count; i++) {
			...
			if (idx >= 0 && idx < this.results.length) {
				const result = this.results[idx];
				if (result && !result.data) result.load();
			}
		}
	}
	scheduleLoadingAnnouncement() {
	...
	handleResultLoaded() {
		...
		while (this.pendingNavigation !== 0) if (direction45 > 0) {
	...
	updateSelectionUI(scroll = false) {
		...
		const activeEl = (result ? this.getOptionsForResult(result) : [])[this.activeOptionOffset];
		if (activeEl) {
			...
			if (scroll) this.scrollToCenter(activeEl);
	...
	getResultAndOffsetFromElement(el) {
		...
		const resultIndex = parseInt(option.getAttribute("data-pf-result-index"), 10);
		const optionOffset = parseInt(option.getAttribute("data-pf-option-offset") || "0", 10);
	...
	activateCurrentSelection(keyboardEvent) {
		...
		const activeEl = this.getOptionsForResult(result)[this.activeOptionOffset];
		if (!activeEl || !activeEl.href) return;
		if (keyboardEvent.metaKey || keyboardEvent.ctrlKey) window.open(activeEl.href, "_blank");
		else if (keyboardEvent.shiftKey) window.open(activeEl.href, "_blank");
		else window.location.href = activeEl.href;
		this.closeDropdown();
	}
	handleResults(searchResult) {
		this.isLoading = false;
		if (this.resultsEl) this.resultsEl.removeAttribute("aria-busy");
		...
		this.results = limitedResults.map((rawResult, index) => {
			const placeholderEl = templateNodes2(this.getPlaceholder())[0];
			if (this.resultsEl && placeholderEl) this.resultsEl.appendChild(placeholderEl);
			const result = new SearchboxResult({
				...
				index,
				onLoad: () => {
					if (this.results[index] === result) this.handleResultLoaded();
				}
			});
			return result;
		});
	...
	*/
	getResultRenderer() {
		...
		return (result) => {
			return defaultResultTemplate2(this.buildTemplateData(result));
		};
	}
	rerenderLoadedResults() {
		...
		for (let i = 0; i < this.results.length; i++) {
			...
			if (result.data) {
				...
				let templateResult;
				if (this.resultTemplate) templateResult = this.resultTemplate(result.data);
				else if (this.compiledResultTemplate) templateResult = this.compiledResultTemplate(templateData);
				else templateResult = defaultResultTemplate2(templateData);
				...
				for (const node of nodes.slice(1)) this.resultsEl.appendChild(node);
			} else {
				const placeholderEl = templateNodes2(this.getPlaceholder())[0];
				if (placeholderEl) {
	...
	announceResults() {
		...
		if (count === 0) this.instance?.announce("zero_results", { SEARCH_TERM: this.searchTerm }, "assertive");
		else {
	...
	register(instance) {
		...
		instance.on("translations", () => {
			...
			this.render();
			if (this.inputEl && currentValue) this.inputEl.value = currentValue;
			if (wasOpen) {
				this.openDropdown();
				if (this.results.length > 0) {
					this.rerenderLoadedResults();
...
if (!customElements.get("pagefind-searchbox")) customElements.define("pagefind-searchbox", PagefindSearchbox);
...
if (typeof window !== "undefined") window.PagefindComponents = components_exports;
//#endregion
export { Instance, PagefindConfig, PagefindElement, PagefindFilterDropdown, PagefindFilterPane, PagefindInput, PagefindKeyboardHints, PagefindModal, PagefindModalBody, PagefindModalFooter, PagefindModalHeader, PagefindModalTrigger, PagefindResults, PagefindSearchbox, PagefindSummary, configureInstance, getInstanceManager };
